...
% EMNLP 2026 System Demonstrations is single-blind: author names are shown and the
% paper is NOT anonymized. We use "preprint" (non-anonymous + page numbers), NOT
% "review" (which would force "Anonymous ACL submission").
...
analysis, all without modifying either the agent or the server. The demonstration
...
browser interface that executes the \emph{real} three-stage pipeline live, so that
every verdict is computed on the spot and each validation stage is animated with in-place
...
this paper. The system targets the safety of tool-augmented language models, and in
particular indirect prompt injection and tool poisoning, a threat surface that has
become a rapidly growing area of interest at ACL and EMNLP; its detectors are
themselves NLP models (a sequence classifier over tool descriptions and a
token-level classifier over tool responses).
...
scenarios and 200 benign task scenarios. The harness ships with the system and is reproducible with
...
not protect a heterogeneous fleet of servers. Recent MCP-specific tools such as
MCP-Guard \citep{xing2025mcpguard}, IPIGuard \citep{an2025ipiguard}, and DRIFT
advance detection but are research prototypes rather than deployable, transport-level
...
planned. The complete source, the installable package, the reproducible benchmark,
and the demonstration are available at
